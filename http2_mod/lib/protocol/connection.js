...
function purgeWakeups() {
  // execute every method stored in the connection _wakeups.
  // note - it may be wasteful to call _send() more than once.
  // but it is how the original code functioned.
  if (!this._wakeups) return;
  const wakeups = this._wakeups.splice(0, this._wakeups.length);
  wakeups.forEach((wakeup) => {
    if (wakeup && (typeof wakeup === 'function')){
      wakeup();
    }
  });
}
...
Connection.prototype._initializeMultiplexing = function _initializeMultiplexing() {
  ...
  this.on('wakeup', purgeWakeups.bind(this));
};
...
Connection.prototype.addWakeup = function() {
  if (!this._wakeups){
    this._wakeups = [];
  }
  this._wakeups.push(this._send.bind(this));
};
