 ...
 * @param {Boolean} [atWordBoundaries] Whether to truncate string at word boundaries
 * @returns {String} Truncated String
 * @private
 */
Notification.prototype.truncateStringToLength = function (string, length ,  atWordBoundaries) {
    if (atWordBoundaries == true) {
        var firstSpaceIndex = string.indexOf(' ');
        var truncateAt = string.lastIndexOf(' ', length);
        if (truncateAt != -1 ){
            length=truncateAt;
        }
    }
