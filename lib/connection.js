 ...
 * @private
 */
Connection.prototype.bufferNotification = function (template, deviceToken) {
	debug('bufferNotification');
	this.notificationBuffer.push({ template: template, token: deviceToken });
};
...
Connection.prototype.flushBufferedNotifications = function () {
	...
	for (var i = 0; i < end; ++i) {
		...
		var requiredLength = this.notificationBucket.calculateNotificationLength(entry.template, entry.token);
		...
		this.notificationBucket.appendToBuffer(entry.template, entry.token, this.getNextId());
 ...
 * @param {Notification|Object} notificationOrTemplate The notification or template object contains content.
 ...
 */
Connection.prototype.broadcast = function (notificationOrTemplate, callback) {
	debug('broadcast');
	this.showState();
	this.run();

	var template;
	if (notificationOrTemplate.toBinaryTemplate) {
		template = notificationOrTemplate.toBinaryTemplate();
	}  else {
		template = notificationOrTemplate;
	}
	...
	(function pullTokenAndAddNotification() {
		...
		self.addNotification(template, token);
	})();
};
 ...
 */
Connection.prototype.addNotification = function (notificationOrTemplate, token) {
	...
	if (this.deferredBucketSendable.promise.isResolved()) {
		this.bufferNotification(template, token);
		return;
	}

	var requiredLength = this.notificationBucket.calculateNotificationLength(template, token);
	requiredLength += this.notificationBucket.sentinelNotificationLength();
	...
	// add the notification to the buffer.
	this.notificationBucket.appendToBuffer(template, token, this.getNextId());
