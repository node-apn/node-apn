...
Connection.prototype.initialize = function () {
	...
	if(this.options.certData) {
		...
		fs.readFile(this.options.cert, function (err, data) {
			if (err) {
				this.deferredInitialize.reject(new Error(err));
				return;
			}
...
Connection.prototype.connect = function () {
	...
	this.initialize().then(function() {
		...
		this.socket = tls.connect(
			...
			function () {
				if (!this.socket.authorized) {
					this.deferredConnection.reject(new Error(this.socket.authorizationError));
				}
		...
		this.deferredConnection.reject(new Error(error));	
	});
...
Connection.prototype.handleTransmissionError = function (data) {
	if (data[0] == 8) {
		...
		var identifier = util.bytes2int(data.slice(2, 6), 4);
...
Connection.prototype.sendNotification = function (notification) {
	...
	this.connect().then(function() {
		...
		if (this.options.enhanced) {
			...
			// Identifier
			position += util.int2bytes(notification._uid, data, position, 4);
	
			// Expiry
			position += util.int2bytes(notification.expiry, data, position, 4);
		...
		position += util.int2bytes(token.length, data, position, 2);
		position += token.copy(data, position, 0);
		position += util.int2bytes(messageLength, data, position, 2);
		...
		notification.deferred.reject(new Error(error));
	});
