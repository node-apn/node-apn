 ...
 * @config {Number} [bucketLength=8192] The length of the bucket, which contains multiple notifications to be sent in a single transmission. It'll also be the lifetime of the TLS connection in bytes. So, the value should be up to 8192 while it's obsreved unexpectable EPIPE errors, possibly caused by the Apple side's implementation, with greater length of written data.
...
/**
 * Connect to the APN service host.
 * It it not required to call this method explicitly.
 * Connect.addNotification will call this automatically.
 */
...
Connection.prototype.handleTransmissionError = function (data) {
    ...
    var deviceTokenOfId = this.notificationBucket.purgeNotificationUntil(identifier);
	if (deviceTokenOfId === false) {
		...
		sentCount -= this.notificationBucket.notificationCount;
	}
	this.emit('sent', sentCount);
	this.emit('transmissionError', errorCode, deviceTokenOfId || null);
	this.raiseError(errorCode, deviceTokenOfId || null);
};

/**
 * @private
 */
Connection.prototype.raiseError = function(errorCode, deviceToken) {
	...
	if (errorCode instanceof Error) {
		...
		this.emit('rejected', errorCode, deviceToken);
	}
};
 ...
 * @private
 */
Connection.prototype.emitReady = function () {
...
Connection.prototype.updateNotificationWaitingTimer = function () {
	...
	this.flushTimerId = setTimeout(function () {
		debug('updateNotificationWaitingTimer expired; flush now');
		this.flushTimerId = null;
		this.flush();
...
Connection.prototype.addNotification = function (notification, token) {
	...
	if (notification.isCompiled) {
		...
		compiledNotification = notification.compile();
	}
...
/**
 * Count holding notifications(transmitting + unsent).
 */
Connection.prototype.countNotification = function () {
	return 0 < (this.notificationBucket.notificationCount + this.notificationBuffer.length);
};

/**
 * Check whether the connection has one or more notifications.
 * If there is no notification left, it also means the connection is in idle state, not transimitting any data.
 */
Connection.prototype.hasNotification = function () {
	return 0 < this.countNotification();
};
