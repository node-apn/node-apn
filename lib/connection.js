...
Connection.prototype.configureAddress = function() {
	if (!this.options.address) {
		if (this.options.production) {
			this.options.address = "api.push.apple.com";
		}
		else {
			this.options.address = "api.sandbox.push.apple.com";
		}
	}
	else {
		if (this.options.address === "api.push.apple.com") {
 ...
 * @private
 */
Connection.prototype.loadCredentials = function loadCredentials() {
	if (!this.credentialsPromise) {
		debug("Loading Credentials");

		var production = this.options.production;
		this.credentialsPromise = credentials.load(this.options)
			.then(function(credential) {
				var parsed;
				try {
					parsed = credentials.parse(credential);
				}
				catch (e) {
					debug(e);
					return credential;
