...
/**
 * Begin bulksend notifications to the APN service, using pull notification contents model.
 * @param {Function} callback  A callback to traverse notifications and device tokens, will be called with no argument. The callback should return an array of [notification, token] while there are should-be-sent contents, or any false value if it reaches to the end.
 */
Connection.prototype.bulksend = function (callback) {
	debug('broadcast');
	this.showState();
	this.run();
	...
	(function pullTokenAndAddNotification() {
		...
		self.addNotification(target[0], target[1]);
	})();
};

/**
 * Broadcast same notification content to a bunch of devices via the APN service, , using pull notification contents model.
 * @param {Notification|Object} notification The notification or compiled one.
 * @param {Function} callback A callback to traverse device tokens, will be called with no argument. The callback should return string while there are should-be-sent contents, or false value if it reaches to the end.
 */
