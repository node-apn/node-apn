...
Connection.prototype.createConnection = function() {
	...
	this.connect().delay(100).then(function () {
		if (this.socket.apnRetired) {
			throw new Error("Socket unusable after connection");
		}
		this.failureCount = 0;
 ...
 */
 Connection.prototype.serviceBuffer = function() {
	...
	} while(repeat && socketsAvailable && this.notificationBuffer.length > 0);

	if (!socketsAvailable && !this.initialisingConnection() && this.sockets.length < this.options.maxConnections) {
...
Connection.prototype.socketAvailable = function(socket) {
	if (!socket || !socket.writable || socket.apnRetired || socket.busy || socket.transmissionErrorOccurred) {
		return false;
	}
	return true;
};
