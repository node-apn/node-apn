	...
	do {
		...
		for (var i = this.sockets.length - 1; i >= 0; i--) {
			if(this.socketAvailable(this.sockets[i])) {
				...
				if(i != lastSocket) { console.log("Using: %d %d", i, lastSocket)}
				lastSocket = i;
				break;
			}
		}
...
Connection.prototype.raiseError = function(errorCode, notification) {
	...
	if (notification && typeof notification.notification.errorCallback == 'function' ) {
		notification.notification.errorCallback(errorCode, recipient);
	} else if (typeof this.options.errorCallback == 'function') {
		this.options.errorCallback(errorCode, notification.notification, notification.recipient);
	}
};
