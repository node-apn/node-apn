 ...
 * @param {Object} template	 A notification template in binary format.
 * @param {Buffer} deviceToken A device token.
 */
NotificationBucket.prototype.calculateNotificationLength = function (template, deviceToken) {
	if (this.options.enhanced) {
		return 11 + deviceToken.length + 2 + template.payload.length;
	} else {
		return 3 + deviceToken.length + 2 + template.payload.length;
	}
};
 ...
 */
NotificationBucket.prototype.appendToBuffer = function (template, deviceToken, id) {
	var requriedLength = this.calculateNotificationLength(template, deviceToken);
	...
	if (this.options.enhanced) {
		...
		// Expiry
		buffer.writeUInt32BE(template.expiry, position);
		position += 4;
	} else {
		//Command
		buffer[position] = 0;
	...
	// Payload Length
	buffer.writeUInt16BE(template.payload.length, position);
	position += 2;
	//Payload
	position += template.payload.copy(buffer, position, 0);
