 ...
 * @return false|null|string  deviceToken of `id`. Returns false if the notification of `id` does not exist, returns null if the notification is a sentinel.
 * @private
 */
NotificationBucket.prototype.purgeNotificationFromEnhanceFormattedBufferUntil = function (id) {
	...
	this.traverseEnhancedFormatBuffer(function (head, end, sentId) {
		...
		if (sentId === id) {
			nextPosition = end;
			var tokenLength = this.buffer.readUInt16BE(head + 9);
			if (tokenLength) {
				deviceToken = this.buffer.toString('hex', head + 11, head + 11 + tokenLength);
			}
			return false;
		}
	}.bind(this));
