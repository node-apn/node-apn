...
var Connection = function (optionArgs) {
		...
		if (!self.openingSocket) {
			process.nextTick(function () {
				...
				self.socket.on('data', function (data) {
				        self.isError = handleTransmissionError(data);
				        onFinished(data);
				});
				...
				self.socket.once('close', function (data) {
	...
	var bufferDataForWrite = function (data) {
        ...
        this.sendNotification = function (note,extendedOptions) {
	        options = (!extendedOptions) ? this.options:extendedOptions;
	        var encoding = 'utf8';
		if (note.encoding) {
	...
	var tidyCachedNotes = function () {
       ...
       var handleTransmissionError = function (data,extendedOptions) {
		...
		//  entry to know where to stop when cycling through cachedNotes
	        options = (extendedOptions)?extendedOptions:options;
	        if (data[0] == 8) {
			self.socket.end();
