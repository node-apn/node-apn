...
module.exports = function(dependencies) {
  ...
  Endpoint.prototype._setupHTTP2Pipeline = function _setupHTTP2Pipeline() {
    ...
    const http2Logger = this.options.http2Logger || noopLogger;

    const serializer = new protocol.Serializer(http2Logger);
    const compressor = new protocol.Compressor(http2Logger, "REQUEST");
    const deserializer = new protocol.Deserializer(http2Logger);
    const decompressor = new protocol.Decompressor(http2Logger, "RESPONSE");
