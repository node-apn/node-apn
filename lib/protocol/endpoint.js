...
module.exports = function(dependencies) {
  ...
  function Endpoint(options) {
    ...
    this._setupHTTP2KeepAlive(1000 * 60 * 5);
  }
  ...
  Endpoint.prototype._setupHTTP2KeepAlive = function _setupHTTP2KeepAlive (timeout) {
    this._connection.ping(() => {
      this.keepAliveHeartbeat = setTimeout(() => this._setupHTTP2KeepAlive(timeout), timeout);
    });
  };
  ...
  Endpoint.prototype.close = function close() {
    if (this._acquiredStreamSlots === 0) {
      ...
      clearTimeout(this.keepAliveHeartbeat);
    }
