...
module.exports = function(dependencies) {
  ...
  Endpoint.prototype._error = function error(err) {
    ...
    setImmediate(() => {
      for(let key in this._connection._streamIds) {
        if (key == 0) continue;
        this._connection._streamIds[key].emit("error", err);
      }
    });
  }
